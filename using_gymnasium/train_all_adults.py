...
def main():
    ...
    # ### <<< UPDATED: Reduced episodes for faster multi-patient testing
    max_episodes = 50 # Use a smaller number for testing, increase for full training (e.g., 2000)
    lr = 3e-4
    gamma_val = 0.99
    tau = 0.005
    alpha = 0.2
    ...
    # --- 3. Define the list of patients to train on ---
    adult_patients = [f'adult#{i:03d}' for i in range(1, 11)]

    # --- 4. Main training loop for each patient ---
    all_patient_results = []
    for patient_name in adult_patients:
        ...
        # --- Setup Directories for each patient ---
        AGENT_NAME = 'sac'
        ...
        # --- Environment Registration for the current patient ---
        ...
        # Use a try-except block in case the env is registered in a previous run
        try:
            register(
                id=env_id,
        ...
        for i_episode in range(1, max_episodes + 1):
            ...
            for t in range(max_timesteps_per_episode):
                if total_timesteps_taken < learning_starts:
                    # Sample from a much safer, smaller range during initial exploration
                    action = np.array([np.random.uniform(low=0, high=0.5)]) # NEW - Recommended
                else:
        ...
        # --- Evaluation for the current patient ---
    ...
    # Convert the collected results into a pandas DataFrame for easy analysis
    import pandas as pd
    results_df = pd.DataFrame(all_patient_results)
    
    # Set patient as the index for cleaner display
    results_df.set_index('Patient', inplace=True)

    print("\n--- Detailed Results Per Patient ---")
    print(results_df)

    # Calculate and print the average performance across the cohort
